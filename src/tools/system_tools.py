...
def list_files(directory: str) -> str:
    """Lists files and directories in a specified location using shortcuts like 'desktop', 'documents', etc.
...
def open_app(application_name: str) -> str:
    """
    Intelligently finds and opens an application on Windows, macOS, or Linux.
...
def get_clipboard_content() -> str:
    """Reads and returns the current content of the system clipboard. Works on Windows, macOS, and Linux."""
    try:
...
def take_screenshot(path: str = "desktop") -> str:
    ...
    Works on Windows, macOS, and Linux. Defaults to saving on the desktop if no path is given.
    Note: On mobile devices, this requires browser permissions.
    """
...
def type_text(text: str) -> str:
    ...
    Works on Windows, macOS, and Linux. Make sure the correct window is active before using.
    Note: On mobile devices, this is not supported due to security restrictions.
    """
    if not HAS_PYAUTOGUI:
...
def get_active_window_title() -> str:
    ...
    Works on Windows, macOS, and Linux.
    Note: On mobile devices, this is not supported.
    """
    if IS_MACOS:
        try:
...
def get_system_info() -> str:
    """
    Returns information about the current system (OS, platform, architecture).
    Useful for determining what tools are available.
    """
    info = {
